  ...
  steps.forEach((step, idx) => {
    ...
    if (step.type === 'keys') {
      if (typeof step.value !== 'string') {
        throw new Error(`${context} step ${idx} key combo must be a string`);
      }
...
export function validateProject(project) {
  ...
  for (const target of project.targets) {
    ...
    if (target.os && !SUPPORTED_OS.includes(target.os)) {
      throw new Error(`unsupported project target os: ${target.os}`);
    }
  }
}
...
export function validateBatch(batch) {
  ...
  for (const job of batch.jobs) {
    ...
    if (job.os && !SUPPORTED_OS.includes(job.os)) {
      throw new Error(`unsupported batch os: ${job.os}`);
    }
  }
}
