...
function parseTerminalCommand(text) {
  ...
  if (includesAny(text, ['linux', 'ubuntu']) && includesAny(text, ['text editor', 'gedit'])) {
    return { templateId: 'linux-terminal-open-text-editor', vars: {} };
  }
...
function parseSpotlight(text) {
  ...
  if (includesAny(text, ['textedit'])) {
    return { templateId: 'mac-spotlight-open-textedit', vars: {} };
  }
